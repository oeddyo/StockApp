...
const myFinanceApp = (state = initState, action) => {
  switch (action.type) {
    case SELECT_STOCK: {
      const after = {
        stocks: [
          ...(state.stocks.slice(0, action.payload).map(x => { return { ...x, selected: false } })),
          { ...state.stocks[action.payload], selected: true },
          ...state.stocks.slice(action.payload + 1).map(x => { return { ...x, selected: false } })
        ]
      }
      return after
    }
